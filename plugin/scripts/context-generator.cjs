...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:g,correlationId:h,...l}=n;Object.keys(l).length>0&&(u=` {${Object.entries(l).map(([f,R])=>`${f}=${R}`).join(", ")}}`)}let T=`[${i}] [${a}] [${_}] ${c}${s}${u}${m}`;if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",u={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},p=new W;var Nt={};function Rt(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,ce.fileURLToPath)(Nt.url))}var Ct=Rt(),C=I.get("CLAUDE_MEM_DATA_DIR"),L=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,de.homedir)(),".claude"),Vt=(0,b.join)(L,"plugins","marketplaces","thedotmack"),qt=(0,b.join)(C,"archives"),Kt=(0,b.join)(C,"logs"),Jt=(0,b.join)(C,"trash"),Qt=(0,b.join)(C,"backups"),zt=(0,b.join)(C,"modes"),Zt=(0,b.join)(C,"settings.json"),pe=(0,b.join)(C,"claude-mem.db"),es=(0,b.join)(C,"vector-db"),ts=(0,b.join)(C,"observer-sessions"),ss=(0,b.join)(L,"settings.json"),rs=(0,b.join)(L,"commands"),ns=(0,b.join)(L,"CLAUDE.md");function me(r){(0,_e.mkdirSync)(r,{recursive:!0})}function ue(){return(0,b.join)(Ct,"..")}var le=require("crypto");var At=3e4;function Ee(r,e,t){return(0,le.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function Te(r,e,t){let s=t-At;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=pe){e!==":memory:"&&me(C),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn()}initializeSchema(){this.db.run(`
