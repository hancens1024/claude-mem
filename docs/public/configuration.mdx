...
| `CLAUDE_MEM_PROVIDER`         | `claude`                        | AI provider: `claude`, `gemini`, `openrouter`, or `anthropic-api` |
...
| Setting                              | Default                       | Description                                   |
|--------------------------------------|-------------------------------|-----------------------------------------------|
| `CLAUDE_MEM_ANTHROPIC_API_KEY`       | —                             | Anthropic API key ([get key](https://console.anthropic.com/settings/keys)) |
| `CLAUDE_MEM_ANTHROPIC_MODEL`         | `claude-sonnet-4-20250514`    | Anthropic model name                          |
| `CLAUDE_MEM_ANTHROPIC_BASE_URL`      | `https://api.anthropic.com`   | Custom base URL for Anthropic API proxy       |
| `CLAUDE_MEM_ANTHROPIC_CONCURRENCY`   | `3`                           | Concurrent message processing count           |

**Features:**
- Direct Anthropic Messages API access (no SDK dependency)
- Support for custom base URL (compatible with proxies)
- Concurrent message processing for better performance
- Automatic fallback to Claude SDK on API failure
